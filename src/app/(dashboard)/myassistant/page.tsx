...
async function AssistantsSection() {
  ...
  const primaryLabel = primary?.display_name || (primary?.email ? String(primary.email).split("@")[0] : "Primary");
  const secondaryLabel = secondary?.display_name || (secondary?.email ? String(secondary.email).split("@")[0] : "Secondary");
    ...
    <section className="space-y-2">
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="text-sm">
          ...
          <div className="grid grid-cols-4 px-3 py-2 gap-3">
